...
function getCrtLists(latitude, longitude, page) {
  ...
  $.ajax({
    url: `/charge_ex/php/search_list.php?key=${decordParam}&n=${page}`, // 요청 엔드포인트
    type: "GET", // 요청 방식
    success: function (data) {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        if (items[i].cpStat == "1") {
          imageSrc.push("/charge_ex/img/green.png");
        } else {
          imageSrc.push("/charge_ex/img/red.png");
        }
      }
      ...
      markerInfo.forEach((info) => {
        ...
        info.parentNode.style.border = "white";
      });
